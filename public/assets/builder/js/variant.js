function Variant() {
    ...
    function A(a) {
        Oc("head").append('<link class="viw" href="/assets/builder/theme/css/' + a + '" rel="alternate stylesheet" type="text/css" media="all">')
    }
    ...
    function O(a) {
        ...
        a.css.length > 0 ? b.attr("vmo", a.css) : b.attr("vmo", "vir"), Oc(a.fonts).each(function() {
            b.append(Oc("<img>").attr("src", "/assets/builder/img/fonts/" + ic(this.fontName) + ".png"))
        }), Oc(".vms").append(b)
    }
    ...
    function xa(a) {
        if (rc()) {
            ...
            onShow: function() {
                ...
                Oc(".vdx").html(Oc("#vdp").html()), Oc(".vcp").attr("src", e), Oc(".vcm .veb").text(c ? Oc("." + a).get(0).naturalWidth + "x" + Oc("." + a).get(0).naturalHeight + " pixels" : ""), Oc(".vco").val(e.replace("/assets/builder/thumbnails/", "/assets/builder/img/")), Oc(".vck").val(a), Oc(".vcf").val(f), Oc(".vdx .vdy").each(function() {
        ...
        if (!rc()) {
            ...
            if (Oc.localStorage(Gc + ".gallery.images")) Oc("#vdp").html(Oc.localStorage(Gc + ".gallery.images")), Oc("#vdp div").each(function() {
                ...
                e.forEach(function(e, f) {
                    d = '<div class="vdy"><img class="vcj" delay-src="/assets/builder/thumbnails/' + e + '" vis="' + e + '" onerror="window.mr_variant.defaultImageError(this)"/><span class="vec"></span></div>', f % 3 === 0 ? c.append(d) : f % 2 === 0 ? b.append(d) : a.append(d), Oc("#vdp").append(a).append(b).append(c)
                })
            }
        }
    }
    ...
    function Ba(a) {
        var b = Oc(a).find("img");
        b.attr("src", ""), b.attr("src", "/assets/builder/thumbnails/" + b.attr("vis")), b.css("display", "inline"), Oc(a).removeClass("no-image"), Oc(a).find(".vjn, .vec").html("")
    }
    ...
    function Fa(a) {
        ...
        return rc() ? void mr_variant.wp.editLink(b.attr("href"), b.text(), b.attr("target"), b.attr("vic")) : void Oc(".vcw").modal({
            ...
            onShow: function() {
                Oc(".vgp").addClass("vih");
                var b = Oc("." + a);
                b.hasClass("ven") ? Oc(".vcq").text("for Lightbox Image") : Ia(), Oc(".vcu").val(Oc("." + a).attr("href").replace("/assets/builder/thumbnails/", "/assets/builder/img/")), Oc(".vcv").val(a), Oc(".vcz").removeAttr("selected"), "undefined" != typeof Oc("." + a).attr("target") ? Oc(".vcy").val(Oc("." + a).attr("target")) : Oc(".vcy").val("_self"), setTimeout(function() {
                    Oc(".simplemodal-container").addClass("vko"), Oc(".simplemodal-overlay").addClass("vko")
                }, 100), Wb()
            },
            onClose: function() {
    ...
    function Ga() {
        var a = Oc("." + Oc(".vcv").val()),
            b = Oc(".vcu").val(),
            c = Oc(".vcy").val();
        a.hasClass("ven") && (a.find("img[alt]").length && a.attr("data-title", a.find("img").attr("alt")), "/assets/builder/img/" === b.substring(0, 4) && (b = b.replace("/assets/builder/img/", "/assets/builder/thumbnails/"))), a.removeClass("inner-link").attr("href", b).attr("target", c), -1 !== b.indexOf("#") && a.addClass("inner-link"), Hb()
    }
    ...
    function wb() {
        var a = Oc(".vgm"),
            ...
            c = Oc(".vgi"),
            d = "";
        thumbnailPath = "/assets/builder/img/sections/", rc() && (thumbnailPath = wp_data.config.section_img_url), Oc("#vgg .vim").each(function(a) {
            var c, e = Oc(this),
                f = e.attr("vbp").split(","),
                g = "";
    ...
    Cc = new Date, this.removeIconButtons = !0, rc() && (cd.wp = {}), Oc(".vkg").text(Cc.getFullYear()), ec(), rc() && (window.localStorage.clear(), Oc(".vhe").remove(), Oc(".vpn.vlp").remove(), Oc(".vlj, .vmb.vlp").remove(), Oc(".vpo").removeAttr("title").append('<i class="material-icons">&#xE161;</i>').find("span[data-glyph]").remove(), Oc(".vpo").attr("data-mrv-tooltip", "Save Page"), Oc(".vpo").off("click").on("click", b).on("click", function() {
        ...
        z(), Oc("[delay-src]").each(function(a, b) {
            ...
            var b = a.headString;
            b = b.replace("href=&quot;css/", 'href="/assets/builder/theme/css/').replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"'), Oc("head").prepend(b)
        }), rc() || x()
    });
    try {
        Oc.isFunction(ga)
    ...
    cd.promptEditForm = fd, Oc(document).on("click", ".vdq:not(.vqg)", function() {
        ...
        var a = Oc(this).get(0);
        Oc(".vco").val(Oc(this).attr("src").replace("/assets/builder/thumbnails/", "/assets/builder/img/")), Oc(".vcp").attr("src", Oc(this).attr("src")), Oc(".veb").text(a.naturalWidth + "x" + a.naturalHeight + " pixels"), Oc(".vdz,.vcl").toggleClass("vhr")
